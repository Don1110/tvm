#ifndef INCLUDE_SDAA_TECO_DETAIL_TECO_SDAA_VECTOR_TYPES_H_
#define INCLUDE_SDAA_TECO_DETAIL_TECO_SDAA_VECTOR_TYPES_H_
#include <string>
...
struct is_signed<_Tp, true> : public true_or_false_type<_Tp(-1) < _Tp(0)> {}; // NOLINT

template <class _T1, class _T2>
struct is_convertible : public true_or_false_type<__is_convertible_to(_T1, _T2)> {};
...
struct is_integral<hip_impl::Scalar_accessor<T, U, n>> : std::is_integral<T> {};
template <typename T, typename U, unsigned int n>
struct is_floating_point<hip_impl::Scalar_accessor<T, U, n>> : std::is_floating_point<T> {};
}  // Namespace std.

namespace hip_impl {
template <typename T, typename Vector, unsigned int idx> struct Scalar_accessor {
  struct Address {
    const Scalar_accessor* p;

    __HOST_DEVICE__
    operator const T*() const noexcept { return &reinterpret_cast<const T*>(p)[idx]; }
    __HOST_DEVICE__
    operator const T*() const volatile noexcept { return &reinterpret_cast<const T*>(p)[idx]; }
    __HOST_DEVICE__
    operator T*() noexcept { return &reinterpret_cast<T*>(const_cast<Scalar_accessor*>(p))[idx]; }
    __HOST_DEVICE__
    operator T*() volatile noexcept {
      return &reinterpret_cast<T*>(const_cast<Scalar_accessor*>(p))[idx];
    }
  };

  friend inline std::ostream& operator<<(std::ostream& os, const Scalar_accessor& x) noexcept {
    return os << x.data[idx];
  }
  friend inline std::istream& operator>>(std::istream& is, Scalar_accessor& x) noexcept {
    T tmp;
    is >> tmp;
    x.data[idx] = tmp;
  ...
  template <
      typename U,
      typename std::enable_if<!std::is_same<U, T>{} && std::is_enum<U>{} &&
                              std::is_convertible<T,
                                                  typename std::enable_if<std::is_enum<U>::value,
                                                                          std::underlying_type<U>>::
                                                      type::type>{}>::type* = nullptr>
  __HOST_DEVICE__ operator U() const noexcept {
    return static_cast<U>(data[idx]);
  }
  template <
      typename U,
      typename std::enable_if<!std::is_same<U, T>{} && std::is_enum<U>{} &&
                              std::is_convertible<T,
                                                  typename std::enable_if<std::is_enum<U>::value,
                                                                          std::underlying_type<U>>::
                                                      type::type>{}>::type* = nullptr>
  __HOST_DEVICE__ operator U() const volatile noexcept {
    return static_cast<U>(data[idx]);
  }
#endif //__HIP_ENABLE_VECTOR_SCALAR_ACCESSORY_ENUM_CONVERSION__

  __HOST_DEVICE__
  operator T&() noexcept { return reinterpret_cast<T(&)[sizeof(Vector) / sizeof(T)]>(data)[idx]; }
  __HOST_DEVICE__
  operator volatile T&() volatile noexcept {
    return reinterpret_cast<volatile T(&)[sizeof(Vector) / sizeof(T)]>(data)[idx];
  }

  __HOST_DEVICE__
  Address operator&() const noexcept { return Address{this}; } // NOLINT
  ...
  __HOST_DEVICE__ Scalar_accessor& operator+=(U x) noexcept {
    data[idx] += x;
    return *this;
  }
  template <typename U, typename std::enable_if<std::is_convertible<U, T>{}>::type* = nullptr>
  ...
  template <typename U = T,
            typename std::enable_if<std::is_convertible<U, T>{} && std::is_integral<U>{}>::type* =
                nullptr>
  __HOST_DEVICE__ Scalar_accessor& operator%=(U x) noexcept {
    data[idx] %= x;
    return *this;
...
template <typename T> struct HIP_vector_base<T, 4> {
  ...
  __HOST_DEVICE__
  constexpr HIP_vector_base(T x_, T y_, T z_, T w_) noexcept : data{x_, y_, z_, w_} {}
  __HOST_DEVICE__
  constexpr HIP_vector_base(const HIP_vector_base&) = default;
  __HOST_DEVICE__
  constexpr HIP_vector_base(HIP_vector_base&&) = default;
...
template <typename T, unsigned int rank> struct HIP_vector_type : public HIP_vector_base<T, rank> {
  ...
  template <  // TODO: constrain based on type as well.
      typename... Us, typename std::enable_if<(rank > 1) && sizeof...(Us) == rank>::type* = nullptr>
...
template <typename V>
__HOST_DEVICE__ inline constexpr bool _hip_any_zero(const V& x, int n) noexcept {
...
#endif //__MAKE_VECTOR_TYPE__(CUDA_name, T)
...
#endif  // INCLUDE_SDAA_TECO_DETAIL_TECO_SDAA_VECTOR_TYPES_H_
